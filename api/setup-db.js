...
const ALL_TABLE_SQL = {
  ...
  tracks: `CREATE TABLE IF NOT EXISTS public.tracks (id TEXT PRIMARY KEY, title TEXT DEFAULT '', tags TEXT DEFAULT '', lyrics TEXT DEFAULT '', audio_url TEXT DEFAULT '', image_url TEXT DEFAULT '', video_url TEXT DEFAULT '', model TEXT DEFAULT '', prompt TEXT DEFAULT '', user_name TEXT DEFAULT '', user_provider TEXT DEFAULT '', is_public BOOLEAN DEFAULT TRUE, play_count INTEGER DEFAULT 0, like_count INTEGER DEFAULT 0, dislike_count INTEGER DEFAULT 0, duration INTEGER DEFAULT 0, task_id TEXT DEFAULT '', gen_mode TEXT DEFAULT 'custom', parent_id TEXT DEFAULT NULL, created_at TIMESTAMPTZ DEFAULT NOW())`,
  users: `CREATE TABLE IF NOT EXISTS public.users (id BIGSERIAL PRIMARY KEY, uid TEXT NOT NULL, name TEXT DEFAULT '', email TEXT DEFAULT '', avatar TEXT DEFAULT '', provider TEXT NOT NULL, plan TEXT DEFAULT 'free', plan_expires TIMESTAMPTZ, credits_song INTEGER DEFAULT 5, credits_mv INTEGER DEFAULT 0, credits_lyrics INTEGER DEFAULT 5, ua TEXT DEFAULT '', is_mobile BOOLEAN DEFAULT FALSE, login_count INTEGER DEFAULT 1, last_login BIGINT DEFAULT 0, session_id TEXT DEFAULT '', session_device TEXT DEFAULT '', session_ip TEXT DEFAULT '', session_at BIGINT DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(uid, provider))`,
  ...
  push_subscriptions: `CREATE TABLE IF NOT EXISTS public.push_subscriptions (id BIGSERIAL PRIMARY KEY, user_name TEXT NOT NULL, user_provider TEXT NOT NULL, subscription JSONB NOT NULL DEFAULT '{}', created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(user_name, user_provider))`,
  user_prefs: `CREATE TABLE IF NOT EXISTS public.user_prefs (id BIGSERIAL PRIMARY KEY, user_name TEXT NOT NULL, user_provider TEXT NOT NULL, genres JSONB DEFAULT '[]', moods JSONB DEFAULT '[]', push_time TEXT DEFAULT '09:00', daily_push_on BOOLEAN DEFAULT FALSE, last_sent_at TIMESTAMPTZ, last_track_id TEXT DEFAULT '', updated_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(user_name, user_provider))`,
};
